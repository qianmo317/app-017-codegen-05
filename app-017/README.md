...
| 教材目录 | 自动识别章/节标题，按出现先后连续编号，生成可触摸目录；目录插入正文前并迭代重算页码，标题续行完整保留，无法稳定时明确报告来回跳的条目 |
...
```bash
npm test           # 单元测试（vitest）：转换/换行/目录/BRF 校验/性能，287 条
...
- 单元测试覆盖：200+ 条转换用例、30 组换行/分页用例、教材目录页码稳定与长标题续行、BRF 结构校验、1 万字 < 300ms 性能
...
│   │   ├── layout.ts               #   分页排版（词不跨行、缩进、页码、可触摸目录）
│   │   ├── headings.ts             #   章/节标题识别
│   │   ├── toc.ts                  #   目录内容构造
